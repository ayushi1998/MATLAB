...
calcCGCoords(const double2* curve, const double2* pts, double2* coords, int M, int N) {
    ...
    double2 res1 = complexdiv(Bjp1, Bj);
    double2 res2 = complexdiv(Bj, Bjm1);
    double2 log1 = complexlog(res1);
    double2 log2 = complexlog(res2);

    if (Bjp1.x*Bjp1.x + Bjp1.y*Bjp1.y < 1e-9) {
        log1.x = 0;
        log1.y = 0;
    }
    if (Bj.x*Bj.x + Bj.y*Bj.y < 1e-9) {
        double2 f1 = complexlog(complexdiv(Bjp1, Bjm1));
        coords[j * M + i].x = (f1.y) / (2*M_PI);
        coords[j * M + i].y = -(f1.x) / (2*M_PI);
        return;
    }
    if (Bjm1.x*Bjm1.x + Bjm1.y*Bjm1.y < 1e-9) {
        log2.x = 0;
        log2.y = 0;
    }
	...
	double2 f1 = complexmul(complexdiv(Bjp1, Ajp1), log1);
	double2 f2 = complexmul(complexdiv(Bjm1, Aj), log2);
